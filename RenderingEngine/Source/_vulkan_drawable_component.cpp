...
void VulkanDrawableComponent::Update(float delta)
{
    ...
    // Prev: mRenderer->GetAppTime()->TotalAppTimeMilliseconds()) + delta
    float red = glm::sin(baseTime + glm::pi<float>() / 4.0f);
    float green = glm::sin(baseTime + 3.0f * glm::pi<float>() / 4.0f);
    float blue = glm::sin(baseTime + 3.0f * glm::pi<float>() / 2.0f);

    glm::vec3 mFilteredColor = glm::vec3(red, green, blue);
}

void VulkanDrawableComponent::Draw(const std::shared_ptr<Camera>& camera)
{
    mUbo.model = mSceneComponent->GetWorldMatrix();
    mUbo.view = camera->ViewMatrix();
    mUbo.proj = camera->ProjectionMatrix();
    // Flip Y-axis for Vulkan clip space
    mUbo.proj[1][1] *= -1;
    mUbo.filteredColor = mFilteredColor;
    ...
    vkCmdBindDescriptorSets(commandBuffersRef[currentFrame], VK_PIPELINE_BIND_POINT_GRAPHICS, 
        mRenderer->GetGraphicsPipeline(mMaterialName).first, 0, 1, &mDescriptorSets[currentFrame], 0, nullptr);
