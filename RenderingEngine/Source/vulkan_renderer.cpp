...
void VulkanRenderer::CreateGraphicsPipeline(Material* material, std::vector<char>& spvVertShaderCode, std::vector<char>& spvFragShaderCode)
{
    VkPipelineLayout pipelineLayout;
    VkPipeline pipeline;
    ...
    {
        if (material->GetMaterialSettings().shadingModel == ShadingModel::Unlit)
        {
            bindingDescription = GetBindingDescription<VertexPositionTextureColor>();
            attributeDescriptions = GetAttributeDescriptions<VertexPositionTextureColor>();
        }
        if (material->GetMaterialSettings().shadingModel == ShadingModel::Lit)
        {
            bindingDescription = GetBindingDescription<VertexPositionTextureColorNormalTangent>();
            attributeDescriptions = GetAttributeDescriptions<VertexPositionTextureColorNormalTangent>();
        }
    }
    ...
    mGraphicsPipelines.emplace(material->GetMaterialSettings().materialName, std::pair<VkPipelineLayout, VkPipeline>(pipelineLayout, pipeline));
    //Destroy shader modules
    vkDestroyShaderModule(mLogicalDevice, fragShaderModule, nullptr);
    vkDestroyShaderModule(mLogicalDevice, vertShaderModule, nullptr);
}
