...
VkImageView RenderingApplication::CreateImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags, std::uint32_t mipmapLevels)
{
...
void RenderingApplication::CreateDepthResources()
{
    VkFormat depthFormat = FindDepthFormat();

    CreateVulkanImage(mSwapChainExtent.width, mSwapChainExtent.height, 1, depthFormat, 
...
VkFormat RenderingApplication::FindSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features)
{
...
VkFormat RenderingApplication::FindDepthFormat()
{
...
bool RenderingApplication::HasStencilComponent(VkFormat format)
{
...
void RenderingApplication::CreateVulkanImage(uint32_t width, uint32_t height, std::uint32_t mipmapLevels, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, 
...
void RenderingApplication::CreateTextureImage()
{
    ...
    std::cout << "Mipmap levels: " << std::to_string(mMipmapLevels) << std::endl;
    ...
    CreateVulkanImage(width, height, mMipmapLevels, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_SRC_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                       VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, mTextureImage, mTextureImageMemory);

    TransitionImageLayout(mTextureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, mMipmapLevels);
    ...
    GenerateMipmaps( mTextureImage, VK_FORMAT_R8G8B8A8_SRGB, width, height, mMipmapLevels );
}
...
void RenderingApplication::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels)
{
...
void RenderingApplication::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, std::uint32_t mipmapLevels)
{
    VkCommandBuffer commandBuffer = BeginSingleTimeCommand();
